...
describe("Check replace cell text", () => {
  test("Basic test", async () => {
    ...
    expect(
      replaceCellText(cell, "c2", "c2-replaced", (s: string, o: string, n: string) =>
        s.replace(o, n),
      ).replacedText,
    ).toEqual(undefined);
  });
  ...
  test("HTML test", async () => {
    ...
    cell.text = htmlText;
    expect(
      replaceCellText(cell, "work", "working", (s: string, o: string, n: string) => s.replace(o, n))
        .replacedText,
    ).toEqual(
      // eslint-disable-next-line quotes
    ...
    const notReplaced = replaceCellText(
      cell,
      "Notes  work",
      "Notes  working",
