...
export const AioSingleReplacements = ({
  ...
  const returnData = useCallback((ret: AioExternalSingle, i: number) => {
  ...
  const updateReplacement = useCallback((ret: { oldText?: string, newText?: string }, i: number) => {
    if (typeof setReplacements !== "function" || replacements === undefined || replacements.length < i - 1) return;
    ...
    <>
      ...
      <div>
        {typeof setReplacements === "function" &&
          <AioIconButton iconName={"aiox-addDown"} onClick={() => addReplacement(0)} tipText={"Add text"} />
        }
            ...
            <div key={repl.airid ?? i}>
              ...
              {typeof setReplacements === "function" &&
                <div className="aiox-button-holder" style={{ display: "flex", flexDirection: "row", alignContent: "center", marginBottom: '2px' }}>
                  {replacements!.length >= 1 && <AioIconButton iconName={"aiox-removeUp"} onClick={() => removeReplacement(i)} tipText={"Remove text"} />}
                  <AioIconButton iconName={"aiox-addDown"} onClick={() => addReplacement(i + 1)} tipText={"Add text"} />
                </div>
              }
            </div>
          )
