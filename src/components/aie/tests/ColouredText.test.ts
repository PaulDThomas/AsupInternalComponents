...
const getMultiSpace = RegExp(/(<(pre|script|style|textarea)[^]+?<\/\2)|(^|>)\s+|\s+(?=<|$)/g);

// Only check basic functions in ColouredText
describe('Check basic ColouredText', () => {
  const testThing = new ColouredText('test two');
  test('Returns text', async () => {
    expect(testThing.text).toEqual('test two');
    expect(testThing.textArray).toEqual(['test two']);
    expect(testThing.htmlString).toEqual(
      '<div class="aie-text"><div class="aie-line"><span class="aie-block">test&nbsp;two</span></div></div>',
    );
  });

  const testThing2 = new ColouredText('test two\r\nline two');
